...
(define (convert-license license)
  (if (member license known-spdx-license-identifiers) license
...
(define (convert-c5-dot-egg egg)
  (append
   ...
   (replicate-one egg 'author string? identity)
   (replicate-one egg 'license string? convert-license)
   (replicate-one egg 'category symbol? symbol->string)
   (replicate-all egg 'dependencies symbol? identity)
   (replicate-all egg 'test-dependencies symbol? identity)))
